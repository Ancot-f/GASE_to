"""Standalone PPCA Chart Builder for GASE-Atlas v3.

Creates ChartStateV3 objects via density-based seeding + local PCA + Mahalanobis radius.
"""
...
class PPCAChartBuilder:
    """Build local charts via density-based seeding + local PCA + radius expansion."""
    ...
    def build_layer_charts(self, features, residuals, labels=None,
        ...
        if self.standardize_features:
            ...
            feat_mean = torch.zeros(D, device=device, dtype=features.dtype)
            feat_std = torch.ones(D, device=device, dtype=features.dtype)
        ...
        if existing_charts:
            class_result = self._classify_samples(
                features, residuals, existing_charts, device)
        ...
        if self.force_one_debug and N >= self.min_samples:
            ...
            if chart is not None:
                ...
                charts.append(chart)
                covered[:] = True
                self._attach_standardizers(charts, feat_mean, feat_std)
        ...
        for attempt in range(self.max_charts):
            ...
            if existing_charts:
                ...
                for old_c in existing_charts:
                    old_within = old_c.within_radius(features)
            ...
            if existing_charts:
                ...
                if max_ol > self.overlap_reject_new and best_old is not None:
                    ...
                    if best_old.adapter is not None:
                        best_old.adapter.to(features_norm.device)
                        with torch.no_grad():
    ...
    def _attach_standardizers(self, charts, feat_mean, feat_std):
        for chart in charts:
            chart.set_feature_standardizer(feat_mean, feat_std, self.standardize_features)
    ...
    def _classify_samples(self, features_norm, residuals, existing_charts, device,
        ...
        for chart in existing_charts:
            if chart.adapter is None:
                continue
            chart.adapter.to(features_norm.device)
            # Geometric distance
